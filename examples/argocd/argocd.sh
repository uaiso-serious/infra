...
kubectl -n argocd apply -f infra/examples/argocd/argocd-custom.yaml
kubectl -n argocd rollout status deployment/argocd-server
kubectl -n argocd scale deployment/argocd-server --replicas 0
kubectl -n argocd rollout status deployment/argocd-server
kubectl -n argocd scale deployment/argocd-server --replicas 1
kubectl -n argocd rollout status deployment/argocd-server
kubectl -n argocd apply -f infra/examples/argocd/argocd-basic.yaml
while true; do
...
kubectl -n onedev exec onedev-0 -- bash -c "curl -s -u admin:admin -d@/tmp/infra/examples/argocd/onedev-new-project.json -H 'Content-Type: application/json' http://onedev.uaiso.lan/~api/projects"
...
kubectl -n argocd apply -f infra/examples/argocd/argocd-uaiso.yaml