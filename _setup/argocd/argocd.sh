...
kubectl -n argocd apply -f infra/_setup/argocd/argocd-custom.yaml
kubectl -n argocd rollout status deployment/argocd-server
kubectl -n argocd scale deployment/argocd-server --replicas 0
kubectl -n argocd rollout status deployment/argocd-server
kubectl -n argocd scale deployment/argocd-server --replicas 1
kubectl -n argocd rollout status deployment/argocd-server
kubectl -n argocd apply -f infra/_setup/argocd/argocd-basic.yaml
while true; do
...
kubectl -n argocd apply -f infra/_setup/argocd/argocd-uaiso.yaml